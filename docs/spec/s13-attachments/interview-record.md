...
**信頼性への影響**:
- REQ-102 (`.attachments/<file_uuid>.bin`) が 🔵 確定
...
**設計判断**: ファイル命名は `.attachments/<file_uuid>.bin` 一階層 (UUID 衝突確率
...
- 重複排除: SHA-256 一致で本体書き込みスキップ (REQ-604)
...
- `EntryRecord.attachment_offset` (u64) は vault.pqd 内の attachment レコード群の起点を指す
...
[filename: utf-8]
[mime_type_len: u16]
[mime_type: utf-8]       // detected via mime_guess or "application/octet-stream"
[size_bytes: u64]
[chunk_count: u32]
[sha256: 32B]            // 元ファイルの SHA-256
[content_hmac: 32B]      // メタデータ全体の HMAC
[signature_len: u32]
[signature: variable]    // ML-DSA-65 over (sha256 || filename || ...)
...
### 5. アトミック書き込み 🟡
- 添付追加: `.attachments/<uuid>.bin.tmp` → rename → vault.pqd 更新
- 失敗時: `.bin.tmp` を zeroize 上書き → 削除
- vault.pqd と `.attachments/` の整合性チェック (起動時 verification command 検討)
...
- メタデータの保存粒度 (filename / MIME / sha256 で足りるか、tag や comment も必要か)
- 添付名重複時の挙動 (rename / skip / overwrite)

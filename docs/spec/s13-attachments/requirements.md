...
を活用しつつ、新 record type 追加に合わせて vault.pqd schema_version は v5 へ更新する。
...
- REQ-102: システムは添付ファイルを OsRng 生成の FileKey と AES-256-GCM で暗号化し、
  `<vault_dir>/.attachments/<blob_uuid>.bin` として保存しなければならない 🟡 *ヒアリング Q2 (vault.pqd + .attachments/) 確定*
- REQ-103: システムは添付ファイルのメタデータ (ファイル名、MIME type、
  サイズ、SHA-256、追加時刻、FileKey) を K_master で暗号化した AttachmentPlaintext として
  vault.pqd 内の attachment レコードに保存しなければならない 🟡 *vault.pqd v5 + 既存 entry 暗号化パターン踏襲*
...
- REQ-302: システムは削除時、同じ blob を参照する attachment が残らない場合に
  `.attachments/<blob_uuid>.bin` を zeroize 上書きしてから削除しなければならない 🔵 *既存 vault::delete_vault の zeroize パターン*
...
- REQ-503: `legacy-access` 実行時、INHERIT 添付のみを新 vault に継承し、
  DESTROY 添付の `.attachments/<blob_uuid>.bin` を、他参照がなければ zeroize 上書きして削除しなければならない 🟡 *S12 セマンティクスの拡張*
...
- REQ-604: import 時、同一 SHA-256 を持つ添付が既に存在する場合は
  既存 blob_uuid + FileKey を共有し、メタデータレコードのみ追加しなければならない 🟡 *重複排除*
...
- REQ-702: `change-password` 実行時、システムは
  vault.pqd の attachment メタデータレコードを新 K_master で再暗号化し、
  `.attachments/*.bin` 本体は FileKey 維持により再暗号化しないものとしなければならない 🟡 *S10 既存パターン拡張*
...
- REQ-901: 添付ファイル本体は `.attachments/<blob_uuid>.bin` の単一階層で保管しなければならない 🟡 *シンプルさ優先*
- REQ-902: 添付メタデータは vault.pqd v5 内の attachment レコード群
  (`RECORD_TYPE_ATTACHMENT=0x03`) として保存し、entry との紐付けは暗号化済み
  `AttachmentPlaintext.entry_uuid` で判定しなければならない 🟡 *v4 予約フィールド活用、既存パターン拡張*
...
- NFR-103: 添付ファイル全体の SHA-256 を計算し、暗号化済み AttachmentPlaintext に保存して
  改ざん検出に使わなければならない 🟡 *データ整合性*
- NFR-104: AttachmentRecord ciphertext の ML-DSA-65 署名は entry record の signature と同等の検証対象としなければならない 🟡 *署名対象拡大、S2 暗号設計と整合*
...
- EDGE-103: 1 エントリで 257 個目の添付追加: REQ-NFR-301 違反、`DiaryError::InvalidArgument` 🟡 *上限制約*
- EDGE-104: 同名添付の重複追加: SHA-256 一致なら既存 blob を共有、不一致なら `DiaryError::InvalidArgument` 🟡 *データ整合性*
...
- EDGE-302: S13 後の vault は `schema_version = 5` とし、古い (S13 以前の)
  クライアントは unsupported schema として安全に拒否しなければならない 🟡 *スキーマ互換性*

...
| legacyブロック | INHERITエントリの平文JSONをK_legacyで暗号化した追加ブロック |
...
```
現在（Phase 1〜3）:
  vault対称鍵を K_master（Argon2id由来）で保護
  + ML-KEM-768で鍵カプセル化

Phase 4（HQC標準化後）:
  vault対称鍵を ML-KEM-768 + HQC ハイブリッドで保護
  → 格子問題・符号理論問題のどちらが破られても安全
```

HQC ハイブリッド対応では vault.pqd のスキーマバージョンをさらに v5 → v6 に更新して対応する。
...
## 6. Vaultファイルフォーマット（v4 / v5、バイナリ）

> **S13 で v5 に更新**: 物理レイアウトは v4 と同一だが、`schema_version` を
> `0x05` に bump し、添付ファイル用の `RECORD_TYPE_ATTACHMENT = 0x03` レコードが
> エントリセクションに混ざる可能性がある。reader は v4 / v5 両方を受理
> (`SCHEMA_VERSION_MIN = 0x04`)。writer は常に v5 を書き込み、v4 vault は
> 次回 write で自動的に v5 化される。詳細は `CHANGELOG.md` の Migration Notes 参照。
...
8       1      スキーマバージョン: 0x05 (S13+ で書き込み、0x04 も読み込み可)
9       1      フラグ（予約: 0x00）
10      2      予約（0x0000）
...
| v4 (S3〜S12) | ✅ (S13+ も対応) | ❌ v5に自動アップグレード |
| v5 (S13+ 現行、添付対応) | ✅ | ✅ |
| v6 (HQC・将来) | — | Phase 4 で実装 |
...
INHERITエントリ:
  エントリ本文をK_masterで暗号化（通常用ciphertext）
  エントリ平文JSONをK_legacyで暗号化（legacyブロック）

DESTROYエントリ:
  エントリ本文をK_masterで暗号化のみ（legacyブロックなし）
```

v4実装はエントリごとのDEKを保持していないため、S12では予約済みの
`legacy_key_block`フィールドにK_legacyで暗号化した平文JSONコピーを格納する。
これによりvault.pqdフォーマットを変更せず、K_masterなしでINHERITエントリを復号できる。
...
2. K_masterでの解錠は試みない
3. 各エントリをスキャン:
   INHERIT → K_legacyでlegacyブロックを復号 → エントリ平文を取得
   DESTROY/未設定 → zeroize・上書き削除
4. INHERITエントリのみを含む新vault.pqdをK_legacyで再暗号化（新しいKEM/DSA鍵を生成）
5. 元vault.pqdを安全に削除
6. K_masterは一切残さない
```
...
- vault.pqdスキーマバージョン v5 → v6 への移行（HQCハイブリッド KEM 対応; S13 までに v5 を採用済み）
